...
{
    /// <summary>
    /// Generic Physical Ammo behaviour to hit something for 2D game.
    /// </summary>
    [RequireComponent(typeof(PhysicalAmmo2DControl))]
    public sealed class PhysicalAmmo2DHitter : MonoBehaviour, IDamagable, IRequiredReset
    {
        #region events
        ...
        [Header("Requirements")]
        [SerializeField]
        private PhysicalAmmo2DControl _controller = null;
...
//#endif
        ...
        public void OnDetectHit(Vector2 originPoint, Vector2 tickVelocity)
        {
            ...
            for (int i = 0; i < _hitCount; i++)
            {
                ...
                {
                    // Ignore self hit.
                    if (_tempHP.EntityID == _controller.EntityID) continue;

                    // Give attack damage at other entity.
                    _tempHP.TakeDamage(tickVelocity, _attackStats.AttackPointDamage, _controller.EntityID);
//#if FMOD
//                    // Run sound if exists.
