    ...
    public class FOV : MonoBehaviour
    {
        ...
        private void Reshape()
        {
            ...
            for (int i = 0; i < raycastCount; i++)
            {
                ...
                _fovHitCount = Physics2D.RaycastNonAlloc(currentWorldPos, direction, _fovHits, _distance, _wallLayer);
                _tempPoints[i + 1] = _fovHitCount > 0 ? _fovHits[0].point - currentWorldPos : currentLocalPos + direction * _distance;
                _tempOffsetedPoints[i + 1] = vertexOffset + _tempPoints[i + 1];
#if UNITY_EDITOR
                //Debug.Log($"Point {i}; Raycast Point At: {_tempPoints[i + 1]}");
                //Debug.Log($"Hitting Walls Count: {_fovHitCount} ({(_fovHitCount <= 0 ? "NULL" : _fovHits[0].point)})");
                Debug.DrawLine(transform.position, currentWorldPos + _tempPoints[i + 1], Color.yellow);
#endif
